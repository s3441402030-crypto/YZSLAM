...
// --- YOLO 增强显示逻辑：修复版 (解决了 baseLine 未定义错误) ---
{
    std::unique_lock<std::mutex> lock(mMutexPAFinsh);
    cv::Mat globalMask = mpTracker->GetDetector()->mOutMask;

    if (!globalMask.empty() && !mmDetectMap.empty()) {
        // 1. 定义局部颜色映射 Lambda
        auto getLabelColor = [](const std::string& label) -> cv::Scalar {
            if (label == "person")    return cv::Scalar(0, 255, 255);   // 绿色
            if (label == "tvmonitor") return cv::Scalar(0, 0, 255);   // 红色
            if (label == "chair")     return cv::Scalar(255, 0, 0);   // 蓝色
            if (label == "keyboard")  return cv::Scalar(0, 255, 255); // 黄色
            return cv::Scalar(255, 128, 0);                           // 其他颜色
        };

        // 2. 将全图 Mask 缩放到当前显示尺寸
        cv::Mat resizedMask;
        cv::resize(globalMask, resizedMask, toShow.size(), 0, 0, cv::INTER_NEAREST);

        for (auto& [label, areas] : mmDetectMap) {
            cv::Scalar color = getLabelColor(label);

            for (auto& area : areas) {
                // 3. 计算缩放后的坐标
                cv::Rect rect(area.x / trackedImageScale, area.y / trackedImageScale, 
                              area.width / trackedImageScale, area.height / trackedImageScale);
                
                rect &= cv::Rect(0, 0, toShow.cols, toShow.rows); // 边界检查
                if (rect.width <= 0 || rect.height <= 0) continue;

                // --- 4. 局部掩码渲染 (解决乱跑的关键) ---
                cv::Mat maskROI = resizedMask(rect);
                cv::Mat imageROI = toShow(rect);

                cv::Mat binaryMask;
                cv::threshold(maskROI, binaryMask, 128, 255, cv::THRESH_BINARY);

                cv::Mat colorLayer(rect.size(), CV_8UC3, color);
                cv::Mat blendedROI;
                cv::addWeighted(imageROI, 1.0, colorLayer, 0.35, 0.0, blendedROI);

                // 只在掩码区域覆盖颜色
                blendedROI.copyTo(imageROI, binaryMask);

                // --- 5. 绘制边框和标签 (修正了 baseLine 变量名) ---
                cv::rectangle(toShow, rect, color, 2);
                
                int currentBaseLine = 0; // 定义变量
                cv::Size tSize = cv::getTextSize(label, cv::FONT_HERSHEY_SIMPLEX, 0.5, 1, &currentBaseLine);
                
                int labelY = std::max(rect.y, tSize.height);
                cv::rectangle(toShow, cv::Point(rect.x, labelY - tSize.height),
                              cv::Point(rect.x + tSize.width, labelY + currentBaseLine), color, cv::FILLED);
                
                cv::putText(toShow, label, cv::Point(rect.x, labelY), 
