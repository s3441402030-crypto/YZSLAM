...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB, const cv::Mat &imD, const double &timestamp, string filename)
{
    ...
    // 2. 获取原始掩码并进行“手术级”自适应扩张
    cv::Mat mask = mpDetector->mOutMask.clone(); 
    ...
    if(!mask.empty())
    {
        ...
        float avgDepth = 0.0f;
        int depthCount = 0;
        // 采样步长设为 10，进一步提升实时性并平滑深度值
        for(int v=0; v<imDepth.rows; v+=10) {
            for(int u=0; u<imDepth.cols; u+=10) {
        ...
        // 物理模型计算：L_real 缩减到 0.05m (5厘米)，这是防止“致盲”的关键
        float fx = mK.at<float>(0,0); 
        int dilationSize = cvRound((fx * 0.05f) / avgDepth); 

        // 严格约束：最小扩张 3 像素，最大仅允许 10 像素
        dilationSize = std::max(3, std::min(20, dilationSize));
        ...
        // 统计调试打印 (可选，确认扩张确实生效且在 3-10 之间)
        // std::cout << "[Level 2 Refined] R: " << dilationSize << "px | D: " << avgDepth << "m" << std::endl;
    }

    // 3. 将精修后的掩码交给提取器
    mpORBextractorLeft->mSegmentationMask = mask;
    ...
    if (mSensor == System::RGBD)
        mCurrentFrame = Frame(mImGray, imDepth, timestamp, mpORBextractorLeft, 
                              ...
                              static_cast<Frame*>(NULL), IMU::Calib(), mask);
    else if(mSensor == System::IMU_RGBD)
        mCurrentFrame = Frame(mImGray, imDepth, timestamp, mpORBextractorLeft, 
                              mpORBVocabulary, mK, mDistCoef, mbf, mThDepth, mpCamera, 
                              &mLastFrame, *mpImuCalib, mask);

    // 打印第一级完成的统计
    int rejectedCount = mpORBextractorLeft->mnLastNumRejected;
    //if(rejectedCount > 0) {
    //    std::cout << "\033[1;33m[Frame: " << mCurrentFrame.mnId << "]\033[0m "
     //             << "Rejected: " << rejectedCount << " pts | "
    //              << "Kept: " << (int)mCurrentFrame.mvKeys.size() << " pts" << std::endl;
    //}
